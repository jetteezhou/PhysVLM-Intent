# 项目结构说明

## 📁 目录结构

```
PhysVLM-Intent/
├── pipeline/                    # Pipeline核心模块
│   ├── __init__.py
│   ├── pipeline.py             # 主Pipeline类（IntentLabelPipeline）
│   ├── audio_processor.py      # 音频处理模块（ASR）
│   ├── video_processor.py      # 视频处理模块（分割、帧采样）
│   └── llm_client.py          # LLM客户端模块（意图分析、目标定位）
│
├── config/                      # 配置模块
│   ├── __init__.py
│   └── settings.py             # 配置和常量
│
├── utils/                       # 工具函数模块
│   ├── __init__.py
│   └── image_utils.py          # 图像处理工具
│
├── tools/                       # 工具和脚本
│   ├── annotation/             # 标注工具（独立服务器，用于单元测试）
│   │   ├── annotation_server.py      # Flask服务器
│   │   ├── annotation_tool.html      # 前端页面
│   │   ├── start_annotation_tool.py  # 启动脚本
│   │   └── requirements_annotation.txt
│   ├── data_collection/        # 数据采集工具（独立服务器，用于单元测试）
│   │   ├── collection_server.py
│   │   ├── collection_tool.html
│   │   ├── start_collection_tool.py
│   │   └── task_config/        # 配置文件目录
│   └── sam/                    # SAM分割工具（实验性）
│       └── sam_test.py         # SAM测试脚本
│
├── docs/                        # 文档
│   ├── README_PIPELINE.md      # Pipeline使用说明
│   ├── README_annotation.md    # 标注工具使用说明
│   └── QUICK_START.md          # 快速开始指南
│
├── unified_server.py            # 统一Web应用服务器（推荐使用）
├── unified_app.html             # 统一Web应用前端页面
├── start_unified_app.py         # 启动统一Web应用（便捷脚本）
├── start_pipeline.py            # 启动标注生成Pipeline（单元测试用）
├── start_annotation.py          # 启动标注检验工具（单元测试用）
├── start_collection.py          # 启动数据采集工具（单元测试用）
├── data_label_gen_pipeline.py   # Pipeline主入口文件（向后兼容）
├── web_html/                    # Web前端页面（用于统一应用）
│   ├── collection_tool.html     # 数据采集工具页面
│   └── annotation_tool.html    # 标注工具页面
├── requirements.txt             # 项目依赖
└── README.md                    # 项目主文档
```

## 📂 目录说明

### pipeline/
Pipeline核心功能模块，包含完整的意图推理和目标定位流程。

### config/
配置管理模块，统一管理API密钥、模型配置等。

### utils/
通用工具函数模块。

### tools/
各种工具和脚本：
- **annotation/**: Web标注工具，用于人工检查和修正AI生成的结果（独立服务器，用于单元测试）
- **data_collection/**: 数据采集工具，管理采集任务和视频数据（独立服务器，用于单元测试）
- **sam/**: SAM（Segment Anything Model）视频分割工具（实验性功能）

### docs/
项目文档目录。

### models/
模型文件存储目录。

## 🚀 使用方式

### 使用统一Web应用（推荐）⭐

```bash
python start_unified_app.py
```

访问 http://localhost:5001 打开统一应用界面，包含数据采集、标注生成和标注检验三个模块。

### 单独运行各个模块（单元测试）

如果需要单独测试某个模块，可以使用以下脚本：

#### 运行Pipeline

```bash
python start_pipeline.py
```

或使用向后兼容的入口：

```bash
python data_label_gen_pipeline.py
```

#### 启动标注工具

```bash
# 方式1：使用便捷脚本
python start_annotation.py

# 方式2：直接运行
python tools/annotation/start_annotation_tool.py
```

访问 http://localhost:5001 进行标注检验。

#### 启动数据采集工具

```bash
python start_collection.py
```

访问 http://localhost:5001 进行数据采集。

### 运行SAM测试（实验性）

```bash
python tools/sam/sam_test.py
```

## 📝 注意事项

1. **路径引用**：所有工具脚本都已更新为使用项目根目录的相对路径，确保在任何位置运行都能正常工作。

2. **数据文件**：Pipeline生成的数据文件位于 `pipeline/outputs/` 目录。

3. **输出目录**：
   - `pipeline/outputs/`: Pipeline输出数据
   - `pipeline/outputs/output_frames/`: Pipeline生成的视频帧图像
   - `annotation_backups/`: 标注工具的备份文件
   - `tools/data_collection/datas/`: 采集的视频数据存储目录
   - `tools/data_collection/task_config/`: 数据采集工具配置文件
   - `runs/`: SAM工具的输出结果（如果使用）

